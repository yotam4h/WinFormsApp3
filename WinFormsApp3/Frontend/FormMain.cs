    ...
    public partial class FormMain : MaterialForm
    {
        ...
        private void materialTabControl1_Selected(object sender, TabControlEventArgs e)
        {
            ...
            else
            {
                ...
                if (curTab == Coffee)
                {
                    comboBoxMilkType.Enabled = false;
                    dataGridViewMain.DataSource = MenuManager.GetMenuItemsByChild<Coffee>();
                }
            }
        }

        private void comboBoxCoffeeType_SelectionChangeCommitted(object sender, EventArgs e)
        {
            if (comboBoxCoffeeType.SelectedItem.ToString() == "Espresso")
            {
                comboBoxMilkType.Enabled = false;
                comboBoxMilkType.Text = "None";
            }
            else
            {
                comboBoxMilkType.Enabled = true;
            }
        }
