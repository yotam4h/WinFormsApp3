...
namespace WinFormsApp3.Backend.Models
{
    ...
    {   
        ...
        public override string ToString()
        {
            ...
            // Add ingredients
            if(Lime_juice || Soda || Mint || Syrup || Sugar_water || Ice) { text += " - "; }
            if(Lime_juice) { text += "Lime juice, "; }
            if (Soda) { text += "Soda, "; }
            if (Mint) { text += "Mint, "; }
            if (Syrup) { text += "Syrup, "; }
            if (Sugar_water) { text += "Sugar water, "; }
            if (Ice) { text += "Ice, "; }
            if (CustomIngredient.Count > 0) { text += string.Join(", ", CustomIngredient); }
